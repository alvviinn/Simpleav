...
if (!isset($_SESSION['payment_phone'])) {
    header("Location: payment2.php?error=missing_phone");
    exit();
}

if (!isset($_SESSION['payment_phone']) || $_SESSION['plan_type'] !== 'basic') {
    header("Location: payment2.php?error=invalid_request");
    exit();
}
...
try {
    ...
    header("Location: payment2.php?error=payment_failed&message=" . urlencode($e->getMessage()));
    exit();
}
